...
export const SuperAdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      ...
      try {
        const response = await fetch('https://eden-hcr-backend.onrender.com/api/admin/metrics', {
          method: 'GET',
          headers: {
  ...
  const handleActionEtablissement = async (id: string, action: 'approuver' | 'rejeter') => {
    setActionMessage(`Traitement de la demande en cours...`);
    // Ici tu feras ton appel FETCH PUT pour modifier le statut en 'actif' ou supprimer
    setTimeout(() => {
      setDemandes(prev => prev.filter(item => item._id !== id));
      setActionMessage(`Établissement mis à jour avec succès.`);
  ...
  return (
    <div className="p-6 lg:p-8 font-sans max-w-7xl mx-auto space-y-8 animate-[fadeInUp_0.4s_ease-out]">
      ...
      {actionMessage && (
        <div className="p-3 text-xs text-eden-navy bg-eden-tan/10 border border-eden-tan/30 rounded-xl animate-pulse">
          {actionMessage}
        </div>
      )}
          ...
          <div className="divide-y divide-eden-border/40 overflow-hidden border border-eden-border/60 rounded-xl bg-eden-bg2/10">
            {demandes.map(item => (
              <div key={item._id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white hover:bg-eden-bg2/20 transition-colors">
                <div className="space-y-1">
                  <p className="text-xs font-bold text-eden-navy">{item.raisonSociale} <span className="text-[10px] font-mono text-eden-tan bg-eden-tan/10 px-1.5 py-0.5 rounded uppercase ml-1">{item.typeEtablissement}</span></p>
