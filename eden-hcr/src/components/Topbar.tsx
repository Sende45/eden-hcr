    ...
    const fetchNotifications = async () => {
      try {
        const res = await fetch(
          `${API_URL}/messagerie/notifications`
        );
  ...
  const handleSearch = async (
    ...
    try {
      const [missionsRes, candidatsRes] = await Promise.all([
        fetch(
          `${API_URL}/mission?search=${encodeURIComponent(value)}`
        ),
        fetch(
          `${API_URL}/candidat?search=${encodeURIComponent(value)}`
        ),
      ]);

      const missionsData = missionsRes.ok
        ? await missionsRes.json()
        : [];

      const candidatsData = candidatsRes.ok
        ? await candidatsRes.json()
        : [];
