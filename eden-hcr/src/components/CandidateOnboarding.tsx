...
// Données renvoyées au parent une fois l'onboarding terminé
...
interface CandidateOnboardingProps {
  // onComplete reçoit maintenant les données du compte créé
...
export const CandidateOnboarding: React.FC<CandidateOnboardingProps> = ({ onComplete }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Si le mot de passe est vide, on génère un temporaire (l'utilisateur
      // pourra le modifier plus tard depuis ses paramètres)
      const password =
        formData.password.trim() ||
        `Eden_${Math.random().toString(36).slice(2, 10)}!`;
      ...
      const registerRes = await fetch(`${API}/api/auth/register`, {
        ...
        body: JSON.stringify({
          ...
          role: 'extra' // rôle par défaut pour un candidat
        })
      });
      ...
      if (!registerRes.ok) {
        setServerError(
          registerData.message || 'Erreur lors de la création du compte.'
        );
        setIsSubmitting(false);
        return;
      }

      // Stocker le token JWT avec la clé unifiée "eden_token"
      const { token, user } = registerData;
      localStorage.setItem('eden_token', token);

      // ── ÉTAPE B : Créer le profil Candidat ─────────────────────────────────
      const candidatPayload = {
        ...
        experience:
          formData.experienceYears === '0-1'
            ? 'sans_experience'
            : formData.experienceYears === '1-3'
            ? '1_2_ans'
            : formData.experienceYears === '3-5'
            ? '3_5_ans'
            : 'plus_5_ans',
      ...
      if (!candidatRes.ok) {
        ...
        // On laisse quand même passer : le profil peut être complété plus tard
        console.warn(
          '[Onboarding] Profil candidat non créé :',
          candidatData.message
        );
      }

      // ── ÉTAPE C : Rediriger vers le dashboard Extra ─────────────────────────
      onComplete({
        token,
        user: {
          id: user.id,
    ...
    <div className="max-w-2xl mx-auto bg-eden-bg2 border border-eden-border rounded-2xl p-8 lg:p-10 shadow-2xl font-sans relative overflow-hidden backdrop-blur-md animate-[fadeInUp_0.4s_ease-out]">
      ...
      <div className="mb-10 select-none relative z-10">
        <div className="flex items-center justify-between text-[11px] font-semibold tracking-widest text-eden-text-light uppercase mb-3.5">
          <span className="bg-eden-navy/5 text-eden-navy px-2.5 py-1 rounded-md font-mono">
            Dossier {step} / 4
          </span>
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-8 relative z-10">
          ...
          <div className="space-y-5 animate-[fadeInUp_0.3s_ease-out]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold tracking-wide text-eden-text-dark">
                  Prénom
                </label>
                <input
                  type="text"
          ...
          <div className="space-y-5 animate-[fadeInUp_0.3s_ease-out]">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                type="text"
                name="city"
                placeholder="Ex: Paris (75) ou Code Postal"
                required
                value={formData.city}
                onChange={handleTextChange}
                className="bg-eden-bg border border-eden-border/80 rounded-xl p-3 text-xs outline-hidden focus:border-eden-tan focus:ring-1 focus:ring-eden-tan/30 transition-all text-eden-text-dark shadow-2xs"
            ...
            <div className="space-y-3">
              <label className="text-xs font-semibold tracking-wide text-eden-text-dark block">
                Jours d'activation pour vos missions
              </label>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                {daysOfWeek.map(day => {
                  ...
                  return (
                    ...
                    >
                      {/* type="checkbox" sans required pour éviter le warning "invalid form control" */}
                      <input
                        type="checkbox"
                        checked={isChecked}
                      ...
                      <span>{day}</span>
                      {isChecked && (
                        <span className="w-1.5 h-1.5 rounded-full bg-eden-tan animate-pulse" />
                      )}
                    </label>
                  );
                })}
        ...
        {step === 4 && (
          <div className="space-y-5 animate-[fadeInUp_0.3s_ease-out]">
            ...
            {/* Pièce d'identité */}
            <div
              className={`border rounded-2xl p-4 flex items-center justify-between gap-4 transition-all duration-300 shadow-2xs
              ${
                formData.idCard
                  ? 'border-eden-teal/40 bg-eden-teal/[0.02]'
                  : 'border-eden-border bg-eden-bg'
              }`}
            >
                ...
                <div
                  className={`p-2.5 rounded-xl ${
                    formData.idCard
                      ...
                      : 'bg-eden-navy/5 text-eden-text-light'
                  }`}
                >
                  <FileText size={18} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-semibold text-eden-text-dark">
                    Pièce d'Identité officielle
                  </p>
                  <p className="text-[11px] text-eden-text-light truncate font-light mt-0.5">
                    {formData.idCard
                      ? formData.idCard.name
                      : 'CNI recto/verso, Passeport ou Titre de séjour'}
                  </p>
                </div>
              </div>
              <label className="shrink-0 p-[7px_14px] bg-eden-navy hover:bg-eden-light-navy text-white rounded-xl text-xs font-medium cursor-pointer transition-colors flex items-center gap-1.5 shadow-sm">
                ...
                {/* Pas de "required" sur les inputs fichiers cachés → évite le warning "invalid form control" */}
                <input
                  type="file"
            ...
            >
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-eden-text-light truncate font-light mt-0.5">
                    {formData.vitaleCard
                      ? formData.vitaleCard.name
                      : "Requis pour la Déclaration Préalable à l'Embauche"}
                  </p>
                </div>
              </div>
              <label className="shrink-0 p-[7px_14px] bg-eden-navy hover:bg-eden-light-navy text-white rounded-xl text-xs font-medium cursor-pointer transition-colors flex items-center gap-1.5 shadow-sm">
                ...
                <input
                  type="file"
                  accept=".pdf,.png,.jpg,.jpeg"
                  onChange={e => handleFileChange(e, 'vitaleCard')}
                  className="hidden"
                />
              </label>
            </div>
              ...
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-eden-text-light truncate font-light mt-0.5">
                    {formData.rib
                      ? formData.rib.name
                      : 'Destiné aux virements sécurisés de vos rémunérations'}
                  </p>
                </div>
              </div>
              <label className="shrink-0 p-[7px_14px] bg-eden-navy hover:bg-eden-light-navy text-white rounded-xl text-xs font-medium cursor-pointer transition-colors flex items-center gap-1.5 shadow-sm">
